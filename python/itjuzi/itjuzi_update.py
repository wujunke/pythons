...
heders = {
            ...
            'Referer':'http://radar.itjuzi.com/focus',
    ...
    def saveCompanyInfoToMongo(self, filepath):
        ...
        with open(filepath) as file:
            ...
            for line in file:
                ...
                aaaa = aaaa + 1
                dic = json.loads(line)
                com_detail,com_name = getCompanyDetail(dic['com_id'])
                if com_name:
...
def getCompanyDetail(com_id):
    ...
    while num > 0:
        try:
            html = requests.get(url_company_detail + '%s'%com_id,headers=headers2, proxies=rand_proxie2()).content
        except ConnectionError:
    ...
    try:
        ...
        com_fullname = soup.find('h1',class_='seo-important-title').text
        com_web = None
        ...
        return response, com_fullname
    except Exception:
        print '抓取详情信息出错'
        print traceback.format_exc()
        return None, com_fullname




class Catch_Thread(threading.Thread):
        def __init__(self, url, path, content, type):
            self.url = url
            self.path = path
                ...
                while aa:
                    try:
                        ...
                        if self.type == 1:
                            ...
                            repeat_count = manager.saveMergeInfoToMongo(self.path)
                        else:
                            repeat_count = manager.saveCompanyInfoToMongo(self.path)
...
Catch_Thread(url_invest_in, temp_path_base+ 'invest_in', '国内投资',1).start()
...
Catch_Thread(url_com, temp_path_base + 'company_in', '国内公司', 3).start()
