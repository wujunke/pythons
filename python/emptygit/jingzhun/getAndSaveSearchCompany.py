...
def getSearchCompany():
    com_list = []
    res = requests.get(base_url + 'mongolog/proj/search').content
    res = json.loads(res)
    if res['code'] == 1000:
...
for proj in projlist:
    ...
    res =  requests.get(jingzhun_search_url + kw, headers=jingzhun_headers).content
    res = json.loads(res)
    pagedata = res['pagedata']
    datalist = pagedata['data']
    for data in datalist:
        if data['phase'] not in ['NONE', u'NONE']:
            time.sleep(10)
            jingzhun_id = data['id']
            tags = data.get('tags',[])
            if "36氪报道" in tags:
