...
def addProj():
    ...
    sql = "SELECT InvestargetDb_v2.dbo.Project.* , InvestargetDb_v2.dbo.ProjectFormat.* FROM InvestargetDb_v2.dbo.Project INNER JOIN InvestargetDb_v2.dbo.ProjectFormat ON InvestargetDb_v2.dbo.Project.ProjectFormatId = InvestargetDb_v2.dbo.ProjectFormat.Id WHERE InvestargetDb_v2.dbo.Project.IsDeleted =0"
    ...
    for row in conn:
        ...
        try:
            ...
            dic = {
                ...
                'publishDate': str(row['PublishedDate']) if row['PublishedDate'] else str(datetime.datetime.now()),
...
def updateproj():
    ...
    for row in conn:
        times = times + 1
        newprojid = datamanager.getNewProjId(row['Id'])
        if newprojid:
            supportuserid = datamanager.getNewUserId(row['UserId'])
            createuserid = datamanager.getNewUserId(row['CreatorUserId'])
            if supportuserid is None:
                supportuserid = 1
            ...
            try:
                ...
                response = requests.put(baseurl + 'proj/%s/'%newprojid, data=json.dumps(dic), headers=headers).content
                response = json.loads(response)
                if response['code'] not in [1000, 2004]:
                    print '新增失败' + str(response)
                    print row['Id']
                else:
                    f = open('projupdate-old_new_id', 'a')
                    print row['Id']
                    print response['result']
                    f.writelines(json.dumps({'old': row['Id'], 'new': response['result']['id']}))
                    f.writelines('\n')
