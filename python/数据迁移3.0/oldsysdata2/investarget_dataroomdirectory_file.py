        ...
        try:
            ...
            if dataroomid:
                dic = {
                    ...
                    'bucket': 'file',
                    'isFile': False,
                    # 'createuser' : userid if userid else 8,
        ...
        try:
            ...
            if dataroomid:
                dic = {
                    ...
                    'orderNO': 1,
                    # 'isShadow': False,
                    # 'shadowdirectory': None,
                    'size': row['Size'],
                    'filename': row['FileName'],
                    'key': row['Key'],
                    'bucket': 'file',
                    'isFile': True,
                    # 'isRead': row['IsRead'] if row['IsRead'] else False,
                    # 'createuser' : userid if userid else 8,
                    # 'createdtime':str(row['CreationTime']),
                    # 'lastmodifytime':str(row['LastModificationTime']),
                    # 'lastmodifyuser':datamanager.getNewUserId(row['LastModifierUserId']),
...
class DataManager():
    ...
    def getAllDirectory(self):
        conn = _mssql.connect(server='101.201.47.50', user='sa', password='Investarget@2016', )
        # SELECT 短链接查询操作（一次查询将所有数据取出）
        sql = "SELECT InvestargetDb_v2.dbo.ProjectDataRoomDirectory.* FROM InvestargetDb_v2.dbo.ProjectDataRoomDirectory INNER JOIN InvestargetDb_v2.dbo.ProjectDataRoom ON ProjectDataRoomDirectory.DataRoomId = ProjectDataRoom.Id WHERE  ProjectDataRoom.IsDeleted = 0 AND ProjectDataRoomDirectory.IsDeleted = 0 AND  ProjectDataRoomDirectory.isShadowFolder = 0"
        conn.execute_query(sql)
        res = []
        for area in conn:
            res.append(area)
    ...
    def getNewDirectoryIdFromFile(self,sqlserverid):
        if sqlserverid:
            mysqlid = None
